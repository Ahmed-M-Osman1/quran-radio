...
function get_radio($get_id, $shownotes=0, $widget_data=''){
	...
	if($RADIO == false){
		...
		if($radio_player == "html5"){
			...
			$code .= '<audio controls="controls"'.$autostart.' src="'.$RADIO['radio'].'/;*.mp3"></audio>';
			$code .= '</div>';
